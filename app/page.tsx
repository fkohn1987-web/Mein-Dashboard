...
type ForecastDetailView = "hourly" | "daily";
...
export default function Home() {
  ...
  const [forecastDetail, setForecastDetail] = useState<ForecastDetailView | null>(null);
  ...
  return (
    <main className="weather-app">
      ...
      <div className="dashboard-shell">
        ...
        {forecastDetail ? <section className="forecast-detail-view" aria-labelledby="forecast-detail-title"><div className="forecast-detail-view__header"><div><p className="eyebrow">Wetterdetails</p><h2 id="forecast-detail-title">{forecastDetail === "hourly" ? "24 Stunden im Detail" : "7 Tage im Detail"}</h2><p className="forecast-detail-view__copy">{forecastDetail === "hourly" ? "Jede Stunde mit Wetterzustand, Temperatur, Niederschlag, Regenrisiko und Wind." : "Jeder Tag mit Wetterzustand, Temperaturspanne, Niederschlag und Sonnenzeiten."}</p></div><button type="button" className="forecast-detail-view__back" onClick={() => setForecastDetail(null)}><ArrowLeft size={16} aria-hidden="true" /> Zur Übersicht</button></div>{forecastDetail === "hourly" ? <HourlyDetailTable weather={weather} /> : <DailyDetailTable weather={weather} />}</section> : <section className="forecast-grid" aria-label="Vorhersage"><article className="forecast-panel forecast-panel--clickable hourly-panel" role="button" tabIndex={0} aria-label="24-Stunden-Verlauf öffnen" onClick={() => setForecastDetail("hourly")} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); setForecastDetail("hourly"); } }}><div className="section-heading section-heading--compact"><div><p className="eyebrow">Nächste Stunden</p><h2>24-Stunden-Verlauf</h2></div><span className="section-count">lokale Zeit</span></div><div className="hourly-scroll">{visibleHourly.length ? visibleHourly.map((hour, index) => <div className={`hour-card ${index === 0 ? "hour-card--now" : ""}`} key={`${hour.time}-${index}`}><span className="hour-label">{index === 0 ? "Jetzt" : formatTime(hour.time, weather?.place.timezone)}</span><span className={`hour-icon hour-icon--${weatherTone(hour.weatherCode)}`}><WeatherIcon code={hour.weatherCode} isDay={weather?.current?.isDay ?? true} size={23} /></span><strong>{formatTemperature(hour.temperatureC)}</strong><span className="hour-rain"><Droplets size={12} aria-hidden="true" />{Math.round(hour.precipitationProbabilityPct)}%</span></div>) : <div className="forecast-empty">Die stündliche Vorhersage wird hier angezeigt, sobald ein Ort geladen ist.</div>}</div><span className="forecast-panel__cta">Stundenwerte öffnen <ArrowRight size={15} aria-hidden="true" /></span></article><article className="forecast-panel forecast-panel--clickable daily-panel" role="button" tabIndex={0} aria-label="7-Tage-Ausblick öffnen" onClick={() => setForecastDetail("daily")} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); setForecastDetail("daily"); } }}><div className="section-heading section-heading--compact"><div><p className="eyebrow">Ausblick</p><h2>Die nächsten 7 Tage</h2></div><span className="section-count">Temperatur & Niederschlag</span></div><div className="daily-list">{visibleDaily.length ? visibleDaily.map((day, index) => <div className={`daily-row ${index === 0 ? "daily-row--today" : ""}`} key={day.date}><span className="daily-date">{index === 0 ? "Heute" : formatDay(day.date, weather?.place.timezone)}</span><span className={`daily-icon daily-icon--${weatherTone(day.weatherCode)}`}><WeatherIcon code={day.weatherCode} size={22} /></span><span className="daily-temps"><strong>{formatTemperature(day.maxC)}</strong><span>{formatTemperature(day.minC)}</span></span><span className="daily-rain"><Droplets size={13} aria-hidden="true" />{Math.round(day.precipitationProbabilityPct)}%</span><span className="daily-amount">{day.precipitationMm.toFixed(1)} mm</span></div>) : <div className="forecast-empty">Der 7-Tage-Ausblick wird hier angezeigt, sobald ein Ort geladen ist.</div>}</div><span className="forecast-panel__cta">Tageswerte öffnen <ArrowRight size={15} aria-hidden="true" /></span></article></section>}
